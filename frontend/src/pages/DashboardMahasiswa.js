...
function DashboardMahasiswa({ token, onLogout }) {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      setErrorStats(null);
      try {
        const res = await fetch(`${Base_Url}/aduan/stats`, {
  ...
  useEffect(() => {
    ...
    const fetchList = async () => {
      ...
      setErrorList(null);
      try {
        const res = await fetch(`${Base_Url}/aduan/user`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!res.ok) {
          const errorText = await res.text();
          throw new Error(errorText || "Gagal mengambil daftar aduan");
        }
        const data = await res.json();
        setListAduan(data);
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-lg shadow-sm p-8 space-y-6 border border-gray-200">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg px-6 py-4">
          ...
          <p className="text-sm text-blue-700">
            Berikut ringkasan aktivitas pengaduanmu.
          </p>
        </div>
        ...
        {!loadingStats && !errorStats && (
          <section className="grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              ...
              <p className="text-3xl font-bold text-red-500">{stats.batal}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <p className="text-sm text-gray-600">Ditolak</p>
              <p className="text-3xl font-bold text-gray-700">{stats.tolak}</p>
            </div>
          </section>
        )}
        ...
        >
          ...
          {loadingList && <p>Loading daftar pengaduan...</p>}
          {errorList && <p className="text-red-500">Error: {errorList}</p>}
          {!loadingList && !errorList && listAduan.length === 0 && (
            <p className="text-gray-600">Belum ada pengaduan.</p>
          )}

          <ComplaintListMahasiswa complaints={listAduan} token={token} />
        </main>
